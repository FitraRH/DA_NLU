document.addEventListener("DOMContentLoaded", () => {
    ...
    function botResponse(userMessage) {
        ...
        setTimeout(() => {
            ...
            if (waitingForDream) {
                addMessage("Thank you for sharing your dream. I'll analyze it for you now.");
                analyzeDream(userMessage);
                waitingForDream = false;
            } else if (waitingForConfirmation) {
                if (userMessage.toLowerCase().match(/yes|yeah|sure|okay|ok|yep|yea/)) {
                    addMessage("Great! Please describe your dream in as much detail as you can remember.");
                    waitingForDream = true;
                    waitingForConfirmation = false;
                } else {
                    addMessage("Alright. If you'd like to analyze a dream later, just let me know.");
                    waitingForConfirmation = false;
                }
            } else {
                const lowerMessage = userMessage.toLowerCase();
                if (lowerMessage.includes("hello") || lowerMessage.includes("hi")) {
                    addMessage("Hello! It's nice to meet you. I'm the Dream Analyzer AI. How can I assist you today?");
                } else if (lowerMessage.includes("analyze") || lowerMessage.includes("dream")) {
                    addMessage("I'm here to help you analyze your dreams. Would you like to share a dream with me for analysis?");
    ...
    function analyzeDream(dreamDescription) {
        ...
        .then(data => {
            removeLoadingMessage();
            if (data.error) {
                addMessage(`I apologize, but there was an error analyzing your dream: ${data.error}`);
            } else {
                displayDreamAnalysis(data);
                addMessage("I hope this analysis provides some insight into your dream. Is there anything else you'd like to know about your dream?");
            }
        })
        .catch(error => {
            removeLoadingMessage();
            addMessage(`I'm sorry, but an error occurred while analyzing your dream: ${error.message}`);
        });
    }
    ...
    resetButton.addEventListener("click", () => {
        ...
        addMessage("Hello! I'm the Dream Analyzer AI. How can I assist you today?");
        waitingForDream = false;
        waitingForConfirmation = false;
    });
